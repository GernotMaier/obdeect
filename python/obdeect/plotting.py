...
def draw_reference_telescope(axis, telescope: str):
    ...
    if telescope == "toy-mst":
        ...
        return
    # Compact projections match the reference-tracer coordinate convention.
    specification = {
        "LST": (11.5, 28.0, None),
        "MST": (6.0, 16.0, None),
    ...
    primary_z = [0.03 * (x / radius) ** 2 for x in primary_x]
    ...
    if secondary_z is not None:
        ...
        axis.plot([-secondary_radius, secondary_radius], [secondary_z, secondary_z], color="0.3",
                  linewidth=2.0, label="M2 reference aperture")
    axis.axhline(camera_z, color="black", linewidth=0.9, label="focal plane")


def main():
    parser = argparse.ArgumentParser(description="Plot an obdeect telescope reference outline and traced paths.")
    ...
    parser.add_argument("--telescope", choices=TELESCOPE_NAMES, default="toy-mst",
    ...
    axis.set(xlabel="telescope x [m]", ylabel="telescope z [m]",
