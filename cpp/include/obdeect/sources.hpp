...
inline std::vector<OpticalPhoton> laser_photons(std::size_t count, double pupil_radius_m,
  ...
  for (std::size_t index = 0; index < positions.size(); ++index) {
    const double u = (static_cast<double>(index) + 0.5) / static_cast<double>(positions.size());
    const double angle = source.divergence_half_angle_rad * std::sqrt(u);
    const double phi = std::fmod(static_cast<double>(index) * 0.6180339887498948482, 1.0) *
                       2.0 * std::numbers::pi;
    ...
    if (!direction) return {};
    photons.push_back({{positions[index], *direction}, static_cast<std::uint64_t>(index), source.wavelength_nm,
                       0.0, 1.0});
  }
  return photons;
}
